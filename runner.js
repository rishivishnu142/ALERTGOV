...
async function waitForPort(port, name, maxRetries = 20) {
...
async function main() {
  ...
  startService('discovery-server', JAVA, [
    '-Xms64m', '-Xmx256m', '-jar',
    path.join(ROOT, 'alertgov-backend', 'discovery-server', 'target', 'discovery-server-1.0.0-SNAPSHOT.jar')
  ...
  for (const svc of services) {
    startService(svc.name, JAVA, [
      '-Xms96m', '-Xmx320m', '-jar',
      path.join(ROOT, 'alertgov-backend', svc.name, 'target', `${svc.name}-1.0.0-SNAPSHOT.jar`)
  ...
  startService('api-gateway', JAVA, [
    '-Xms64m', '-Xmx256m', '-jar',
    path.join(ROOT, 'alertgov-backend', 'api-gateway', 'target', 'api-gateway-1.0.0-SNAPSHOT.jar')
  ...
  console.log('\nChecking service readiness in background...');
  await wait(10000);
